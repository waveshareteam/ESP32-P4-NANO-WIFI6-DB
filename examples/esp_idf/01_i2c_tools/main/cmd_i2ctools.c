...
#define I2C_TOOL_FREQ_STANDARD_HZ (100 * 1000)
#define I2C_TOOL_FREQ_FAST_HZ     (400 * 1000)
#if 0//CONFIG_EXAMPLE_I2C_SPEED_400KHZ
static uint32_t i2c_frequency = I2C_TOOL_FREQ_FAST_HZ;
#else
static uint32_t i2c_frequency = I2C_TOOL_FREQ_STANDARD_HZ;
#endif
i2c_master_bus_handle_t tool_bus_handle;
static TaskHandle_t waveform_task_handle;
static i2c_master_dev_handle_t waveform_dev_handle;
static const uint8_t waveform_data[] = { CONFIG_EXAMPLE_I2C_WAVEFORM_DATA };

static void waveform_task(void *arg)
{
    (void)arg;
    while (true) {
        esp_err_t ret = i2c_master_transmit(waveform_dev_handle, waveform_data,
                                            sizeof(waveform_data), I2C_TOOL_TIMEOUT_VALUE_MS);
        if (ret != ESP_OK && ret != ESP_ERR_INVALID_RESPONSE) {
            ESP_LOGW(TAG, "Waveform transmit failed: %s", esp_err_to_name(ret));
        }
        vTaskDelay(pdMS_TO_TICKS(CONFIG_EXAMPLE_I2C_WAVEFORM_INTERVAL_MS));
    }
}
...
void i2ctools_stop_waveform_task(void)
{
    if (waveform_task_handle != NULL) {
        vTaskDelete(waveform_task_handle);
        waveform_task_handle = NULL;
    }
    if (waveform_dev_handle != NULL) {
        ESP_ERROR_CHECK(i2c_master_bus_rm_device(waveform_dev_handle));
        waveform_dev_handle = NULL;
    }
}
...
void i2ctools_start_waveform_task(void)
{
    ...
    const i2c_device_config_t waveform_config = {
        .scl_speed_hz = i2c_frequency,
        .device_address = CONFIG_EXAMPLE_I2C_WAVEFORM_ADDRESS,
        .flags.disable_ack_check = true,
    };
    ESP_ERROR_CHECK(i2c_master_bus_add_device(tool_bus_handle, &waveform_config, &waveform_dev_handle));
    if (xTaskCreate(waveform_task, "i2c_waveform", 3072, NULL, 5, &waveform_task_handle) != pdPASS) {
        ESP_ERROR_CHECK(ESP_ERR_NO_MEM);
    }
    ESP_LOGI(TAG, "Continuous waveform started: %lu Hz, addr=0x%02x, data=0x%02x",
             (unsigned long)i2c_frequency, CONFIG_EXAMPLE_I2C_WAVEFORM_ADDRESS, waveform_data[0]);
}

static bool i2c_frequency_is_supported(uint32_t frequency)
{
    return frequency == I2C_TOOL_FREQ_STANDARD_HZ || frequency == I2C_TOOL_FREQ_FAST_HZ;
}
...
static int do_i2cconfig_cmd(int argc, char **argv)
{
    ...
    if (i2cconfig_args.freq->count) {
        if (!i2c_frequency_is_supported(i2cconfig_args.freq->ival[0])) {
            ESP_LOGE(TAG, "Only 100000 Hz and 400000 Hz are supported");
            return 1;
        }
    }
    /* Check "--sda" option */
    ...
    ESP_LOGI(TAG, "I2C bus configured at %lu Hz", (unsigned long)i2c_frequency);
    i2ctools_start_waveform_task();
...
static void register_i2cconfig(void)
{
    ...
    i2cconfig_args.freq = arg_int0(NULL, "freq", "<100000|400000>", "Set I2C speed to 100 kHz or 400 kHz");
...
static esp_err_t i2c_probe_at_current_frequency(uint8_t address)
{
    ...
    esp_err_t ret = i2c_master_bus_add_device(tool_bus_handle, &i2c_dev_conf, &dev_handle);
    if (ret != ESP_OK) {
        return ret;
    }

    // A START followed by STOP makes the driver emit only the address byte.
    // Unlike i2c_master_probe(), this device transaction uses i2c_frequency.
    i2c_operation_job_t probe_ops[] = {
        { .command = I2C_MASTER_CMD_START },
        { .command = I2C_MASTER_CMD_STOP },
    };
    ret = i2c_master_execute_defined_operations(dev_handle, probe_ops, 2, I2C_TOOL_TIMEOUT_VALUE_MS);
...
static int do_i2cdetect_cmd(int argc, char **argv)
{
    ...
    for (int i = 0; i < 128; i += 16) {
        ...
        for (int j = 0; j < 16; j++) {
            fflush(stdout);
            address = i + j;
            esp_err_t ret = i2c_probe_at_current_frequency(address);
            if (ret == ESP_OK) {
                printf("%02x ", address);
