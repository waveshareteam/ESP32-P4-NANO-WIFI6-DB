...
void app_main(void)
{
    ...
    for (; ; ) {
        ...
        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "found device id %02X", address);
        } else {
            ESP_LOGI(TAG, "-----------------------------");
        }
        address = (address + 1) % 255;
        vTaskDelay(pdMS_TO_TICKS((int)(1000 / 255)));
    }
    #endif
    ...
    printf(" |                                                            |\n");
    printf(" |  1. Continuous I2C waveform starts automatically             |\n");
    printf(" |  2. Try 'help', check all supported commands               |\n");
