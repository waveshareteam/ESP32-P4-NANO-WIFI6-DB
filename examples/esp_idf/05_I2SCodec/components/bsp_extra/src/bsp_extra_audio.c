...
esp_err_t bsp_extra_codec_set_fs(uint32_t rate, uint32_t bits_cfg, i2s_slot_mode_t ch)
{
    ...
    if (s_play_dev) {
        esp_err_t open_ret = esp_codec_dev_open(s_play_dev, &fs);
        ret |= open_ret;
        if (open_ret == ESP_OK) {
            s_play_opened = true;
        }
    ...
    if (s_record_dev) {
        esp_err_t open_ret = esp_codec_dev_open(s_record_dev, &fs);
        ret |= open_ret;
        if (open_ret == ESP_OK) {
...
esp_err_t bsp_extra_codec_mic_gain_set(float gain_db)
{
    ...
    ESP_RETURN_ON_FALSE(ret == ESP_CODEC_DEV_OK, ESP_FAIL, TAG, "set microphone gain failed");
    return ESP_OK;
}
