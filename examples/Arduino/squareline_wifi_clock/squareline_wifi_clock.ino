...
using selected_lcd_t = squareline_wifi_clock::LCD_HX8394;
#elif ESP_PANEL_DRIVERS_LCD_ENABLE_ILI9881C_LOCAL
#include "src/drivers/lcd/esp_panel_lcd_ili9881c.hpp"
using selected_lcd_t = squareline_wifi_clock::LCD_ILI9881C;
...
using selected_lcd_t = squareline_wifi_clock::LCD_JD9365_LOCAL;
#endif
...
#define LCD_BACKLIGHT_MODE_REGISTER   (0x95)
#define LCD_BACKLIGHT_SETUP_DELAY_MS  (100)
#define LCD_BACKLIGHT_STARTUP_DELAY_MS (1000)
#define LCD_COLOR_BAR_TEST             (0)
...
static LCD *create_native_lcd()
{
    ...
    return new selected_lcd_t(bus, lcd_config);
}
...
static Touch *create_touch(LCD *panel)
{
    BusI2C::Config bus_config = {
        .host_id = BOARD_I2C_PORT,
        ...
        .control_panel = (BusI2C::ControlPanelFullConfig)ESP_PANEL_TOUCH_I2C_CONTROL_PANEL_CONFIG(GT911),
    };
    auto *bus = new BusI2C(bus_config);
    if (!bus->configI2C_HostSkipInit()) {
        delete bus;
        return nullptr;
    }

    auto *new_touch = new TouchGT911(
        bus,
        panel->getFrameWidth(),
    ...
    new_touch->configResetActiveLevel(TOUCH_RST_ACTIVE_LEVEL);
    return new_touch;
}
...
static bool start_display()
{
    ...
    touch = create_touch(lcd);
    if (touch == nullptr) {
        Serial.println("LCD: GT911 creation failed");
        return false;
    }
    if (!check_result(touch->begin(), "LCD: GT911 begin failed")) {
        return false;
    }
...
#if LCD_COLOR_BAR_TEST
    Serial.println("LCD: showing MIPI-DSI horizontal color bars");
    if (!check_result(lcd->DSI_ColorBarPatternTest(LCD::DSI_ColorBarPattern::BAR_HORIZONTAL),
                      "LCD: color-bar test failed")) {
        return false;
    }
    while (true) {
        delay(1000);
...
#define weather_enabled         0       // Set to 1 to enable OpenWeather requests
...
void setup()
{
    ...
    if (!start_display()) {
        Serial.println("Display startup failed");
        return;
    }

    Serial.println("Initializing LVGL");
    if (!check_result(lvgl_port_init(lcd, touch), "LVGL: port initialization failed")) {
        return;
    }
